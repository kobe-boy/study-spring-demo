...
public class ArithmeticAspect {
	...
	@Before("execution(public int com.aop.Arithmetic.*(..))")
	...
	@After("execution(public int com.aop.Arithmetic.*(..))")
	...
	//返回通知
	//正常返回通知，可以访问到目标返回值
	@AfterReturning(value="execution(public * com.aop.Arithmetic.*(..))",
			returning="result")
	...
	//在目标方法出现异常时会出现
	@AfterThrowing(value="execution(public * com.aop.Arithmetic.*(..))",
			throwing="ex")
